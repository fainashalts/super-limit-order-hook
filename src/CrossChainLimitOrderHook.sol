...
/**
 * @title CrossChainLimitOrderHook
 * @notice A Uniswap v4 hook that enables limit orders across multiple chains
 * @dev Implements beforeSwap and afterSwap hooks to monitor prices and execute orders
 */
contract CrossChainLimitOrderHook is BaseHook, ReentrancyGuard {
    ...
    function _executeOrder(uint256 orderId) internal {
        ...
        if (order.status != OrderBook.OrderStatus.ACTIVE) revert OrderAlreadyExecuted();
